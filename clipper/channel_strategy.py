...
def _build_prompt(
    ...
    if top_performers:
        if top_performer_basis == "retention":
            lines.append(
                f"\nTOP {len(top_performers)} SHORTS TO MODEL (ranked by retention, "
            ...
            lines.append(
                f"\nTOP {len(top_performers)} SHORTS TO MODEL (ranked by views -- "
    ...
    top_performer_thumbnails = [v for v in (top_performers or []) if v.get("thumbnail_url")]
    top_performer_thumbnails_note = ""
    if top_performer_thumbnails:
        labels = ", ".join(f'"{v["title"]}"' for v in top_performer_thumbnails)
        top_performer_thumbnails_note = (
            f"\n\n{len(top_performer_thumbnails)} actual thumbnail image(s) for the TOP SHORTS "
            f"TO MODEL above are also attached after this message (each preceded by a text label): "
            f"{labels}. Actually look at them for the CONTENT THAT WORKS section below -- name "
            f"the real shared visual pattern (composition, color, close-up face/reaction or not, "
            f"text overlay style and wording) instead of guessing from the titles alone. If they "
            f"don't actually share a pattern, say that instead of forcing one."
        )
    long_form_section = (
...
CONTENT THAT WORKS: if a TOP SHORTS TO MODEL list is given above, base this
on THAT specifically -- name the concrete shared pattern across those exact
videos (topic, title wording/structure, and the thumbnail pattern from the
attached images if given) and say to copy it, not a vague trend from the
channel overall. Without that list, say what type of clip and title/hook
...
def get_ai_overview(
    ...
    # The top Shorts to actively model future titles/thumbnails/hooks on --
    # a different pool (Shorts, not long-form) and a different purpose
    # (name the pattern worth copying, not compare a top/bottom pair).
    ...
    for v in top_performers:
        if not v.get("thumbnail_url"):
            continue
        content.append({"type": "text", "text": f'Thumbnail for Short "{v["title"]}":'})
        content.append({"type": "image", "source": {"type": "url", "url": v["thumbnail_url"]}})
